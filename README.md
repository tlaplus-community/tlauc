...
Rust's type system proved very helpful here; instead of storing indices or offsets as primitive types like `usize`, two generic wrapper types `CharMeasure<T>` and `ByteMeasure<T>` were defined to enforce index arithmetic safety at the type-checking level.
